...
#>
	...
	process
	{
		try { $uri = '{0}{1}' -f (Get-PSFConfigValue -FullName 'GPOTools.Client.Uri' -NotNull), $FunctionName }
		catch { $PSCmdlet.ThrowTerminatingError($_) }
		$header = @{ }
		
		#region Authentication
		$unprotectedToken = Get-PSFConfigValue -FullName 'GPOTools.Client.UnprotectedToken'
		$protectedToken = Get-PSFConfigValue -FullName 'GPOTools.Client.ProtectedToken'
