...
#>
	...
	begin
	{
		...
		$domainName = (Get-ADDomain -Server $pdcEmulator).DNSRoot
	...
	process
	{
		...
		foreach ($adEntity in $Name)
		{
			...
			{
				$identities.AddRange((Get-ADGroup -Server $Domain -LDAPFilter "(name=$adEntity)" | Resolve-ADPrincipal -Domain $Domain))
				continue
			}
