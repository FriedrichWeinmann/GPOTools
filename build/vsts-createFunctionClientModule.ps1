...
$parentModule = 'GPOTools'
if (-not $ModuleName) { $ModuleName = 'GPOTools.Client' }
...
$publishRoot = Join-Path -Path $WorkingDirectory -ChildPath 'publish\GPOTools'
...
$originalManifestData = Import-PowerShellDataFile -Path "$publishRoot\GPOTools.psd1"
$prereqHash = @{
